		...
		if (packet->payload_packet_len > 4 && memcmp(packet->payload, "GET /", 5) == 0) {
			...
			if (packet->parsed_lines == 8
				&& (packet->line[0].offs != 0xffff && packet->line[0].len >= 30
					&& (memcmp(&packet->payload[5], NDPI_STATICSTRING("notice/login_big")) == 0
						|| memcmp(&packet->payload[5], NDPI_STATICSTRING("notice/login_small")) == 0))
				&& memcmp(&packet->payload[packet->line[0].len - 19], "/index.asp HTTP/1.", 18) == 0
				&& (memcmp_packet_hdr(packet,host_line_idx, NDPI_STATICSTRING("crossfire"), 0) == 0 ||
				    memcmp_packet_hdr(packet,host_line_idx, NDPI_STATICSTRING("www.crossfire"), 0) == 0)
				) {
